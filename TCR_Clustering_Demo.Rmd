...
```{r run_clustering_diana_200, echo = FALSE}
seuratObj_CD8s <- RunTcrClustering(
  seuratObj_TCR = seuratObj_CD8s,
  chainsToCluster = NULL, # if null, all assays will be processed
  clusteringMethod = "DIANA",
...
```{r plot_activation_biological_replicates, fig.width = 15, fig.height = 15, warning=FALSE}
ggplot(
  plot_df,
  aes(
    x = cluster_cells + 1,
...
See specifically: this sub-panel (Subject 32726) whose cluster 64 and cluster 260 shows right-skewed (large & activated) populations. 

```{r highlight_clusters_64_260, warning = FALSE}
...
Cluster 64, while containing a large number of activated cells in the VY9 stimulated condition, is also high for cells in the unstimulated condition (purple), suggesting that this cluster contains cytokine producing T cells that are not TCR specific.
...
```{r compare_stats_models, warning = FALSE}
mixed_effects_results %>%
  pivot_longer(
...
```{r compare_stats_cluster_260, warning = FALSE}
mixed_effects_results %>%
  pivot_longer(
    cols = c(PValue_VY9, PValue_AN10),
    names_to = "Stim",
